...
async fn seed_memory(
    ...
    let cfg = MemoryConfig { db_path: db_dir.join("memory.db"), ..MemoryConfig::default() };
...
async fn auto_retrieval_feeds_a_seeded_fact_to_the_real_model() {
    ...
    let (mem_tools, retriever) =
        seed_memory(&workspace, db.path(), "The project's secret deploy codeword is BANANA-7.").await;

    let mut cfg = RuntimeConfig::from_launch("openai".into(), url.clone(), model_name.clone(),
        "native".into(), 262_144);
    cfg.memory = true;
    let sink = Arc::new(Capture::default());
    let built = assemble_loop(&cfg, LoopParts {
        model: Arc::new(OpenAiCompatClient::new(url, model_name, None)),
        sink: sink.clone(), approval: Arc::new(AutoApprove), workspace: workspace.clone(),
        mcp_tools: vec![], memory_tools: mem_tools, memory_retriever: Some(retriever),
    ...
    built.loop_.run(&mut ctx, "What is the secret deploy codeword for this project?".into())
        .await.unwrap();

    let answer = sink.text.lock().unwrap().clone();
    eprintln!("[memory ON] {answer}");
    assert!(answer.to_lowercase().contains("banana"),
...
async fn memory_off_suppresses_recall() {
    ...
    assert!(!answer.to_lowercase().contains("banana-7"),
