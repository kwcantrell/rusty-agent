...
impl MemoryAdmin {
    ...
    fn filter(&self) -> ScopeFilter {
        match &self.scope {
            MemoryScope::Project(k) => ScopeFilter::ProjectAndGlobal {
                project_key: k.clone(),
            },
    ...
    pub async fn update(
        ...
        Ok(MemoryRow {
            ...
            text: rec.text,
            tags: rec.tags,
            scope_kind: rec.scope.kind().into(),
    ...
    pub async fn recall_preview(&self, query: &str) -> Vec<ScoredRow> {
        ...
        match crate::tools::query_memories(
        ...
        .await
        {
            Ok(hits) => hits
                ...
                .map(|h| ScoredRow {
                    ...
                    text: h.record.text,
                    score: h.score,
                    scope_kind: h.record.scope.kind().into(),
                })
                .collect(),
            Err(e) => {
...
pub fn build_tools_with(
    ...
    vec![
        Arc::new(tools::Remember {
            embedder: embedder.clone(),
            store: store.clone(),
            cfg: cfg.clone(),
            project_key: key.clone(),
        }),
        Arc::new(tools::Recall {
            embedder: embedder.clone(),
            store: store.clone(),
            cfg: cfg.clone(),
            project_key: key.clone(),
        }),
        Arc::new(tools::Forget {
            embedder,
            store,
            cfg,
            project_key: key,
...
pub fn build_tools(
    cfg: MemoryConfig,
    workspace: &Path,
) -> Result<Vec<Arc<dyn agent_tools::Tool>>, MemoryInitError> {
    let store =
        SqliteStore::open(&cfg.db_path).map_err(|e| MemoryInitError::Store(e.to_string()))?;
    #[cfg(feature = "onnx")]
    let embedder: Arc<dyn Embedder> = Arc::new(
        embedder::FastEmbedEmbedder::new(&cfg)
            .map_err(|e| MemoryInitError::Embedder(e.to_string()))?,
    );
...
// Legacy lint, unrelated to this branch: the tuple return is clear enough in place.
...
mod admin_tests {
    ...
    async fn admin_lists_and_hides_cross_project_records() {
        use crate::{
            Embedder, InMemoryStore, MemoryConfig, MemoryRecord, MemoryScope, StubEmbedder,
        };
        let embedder: Arc<dyn Embedder> = Arc::new(StubEmbedder::d384());
        ...
        store
            .upsert(MemoryRecord {
                id: "x".into(),
                text: "hi".into(),
                scope: MemoryScope::Project("OTHER".into()),
                tags: vec![],
                vector: v,
                created_at: 1,
                updated_at: 1,
        ...
        assert!(admin.fetch_editable("x").await.unwrap().is_none()); // fetch layer hides it (Ok(None), not Err)
                                                                     // Legacy lint, unrelated to this branch: explicit `, false` documents the bool result.
        #[allow(clippy::bool_assert_comparison)]
        {
            assert_eq!(admin.delete("x").await.unwrap(), false); // silent no-op, not Err
        }
        assert!(admin
            .update("x", Some("new text".into()), None)
            .await
            .is_err()); // "not found"
    }
}
...
mod build_tests {
    ...
    async fn assemble_memory_scopes_to_workspace() {
        ...
        let key_a = match project_scope(Path::new("/tmp/ws-a")) {
            ...
            MemoryScope::Global => unreachable!(),
        };
        let v = embedder
            .embed(&["alpha fact".to_string()])
            .await
            .unwrap()
            .remove(0);
        store
            .upsert(MemoryRecord {
                ...
                scope: MemoryScope::Project(key_a),
                tags: vec![],
                vector: v,
                created_at: now_secs(),
                updated_at: now_secs(),
        ...
        assert!(
            retr_b.retrieve("alpha fact").await.is_empty(),
