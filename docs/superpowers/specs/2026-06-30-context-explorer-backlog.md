...
- ~~**`MemoryAdmin::get` is still production-dead.**~~ **RESOLVED:** the public `get` (a `MemoryRow` projection over `fetch_editable`, with no production caller) was removed. `fetch_editable` remains the single private editable-fetch accessor used by `delete`/`update`; the no-leak test now asserts against `fetch_editable` directly. `MemoryRow` stays (returned by `list`/`update`).
